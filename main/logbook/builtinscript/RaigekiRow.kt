...
)
{
    ...
    for (i in 1..12) {
        ...
        row.add(ydam.toString())
        row.add(if (kabau) "1" else "0")
        if (isSecond) { row.addAll(arg.combinedRows[at-7].updateShipRowBody(prevHP[HP_INDEX_FRIEND_COMBINED][at-7], arg.battle.maxFriendHpCombined[at-7])) }
        else { row.addAll(arg.friendRows[at-1].updateShipRowBody(prevHP[HP_INDEX_FRIEND][at-1], arg.battle.maxFriendHp[at-1])) }
        if (enemyIsSecond) { row.addAll(arg.enemyCombinedRows[df-7].updateShipRowBody(prevHP[HP_INDEX_ENEMY_COMBINED][df-7],arg.battle.maxEnemyHpCombined[df-7])) }
        else { row.addAll(arg.enemyRows[df-1].updateShipRowBody(prevHP[HP_INDEX_ENEMY][df-1],arg.battle.maxEnemyHp[df-1])) }
        row.add(arg.combinedFlagString)
        row.add(if (arg.battle.isEnemyCombined) "連合艦隊" else "通常艦隊")
