...
fun AkakariHougekiRowEXHeader(): ArrayList<String> {
 ...
 */
private fun AkakariHougekiRowEXBodyConstruct(
        arg:ScriptArg,
...
)
{
    ...
    if(arg.battle.isEnemyCombined || arg.isSplitHp){
        AkakariHougekiRowEXBodyConstructEC(arg = arg,attackList = attackList,apiName = apiName,hougekiCount = hougekiCount,startHP = startHP,body = body)
        return
    }
 ...
 */
private fun AkakariHougekiRowEXBodyConstructEC(
        arg:ScriptArg,
        attackList: List<BattleAtackDto>?,
        apiName:String,
        hougekiCount: String,
...
fun AkakariHougekiRowEXBody(arg:ScriptArg): ArrayList<ArrayList<String>> {
    val body = ArrayList<ArrayList<String>>()
    if(arg.hasAkakariInfo) {
        arg.dayPhaseOrNull?.run {
            val phase = this
            AkakariHougekiRowEXBodyConstruct(
                    arg = arg,
