 ...
 *
 */
public abstract class DropOrCombatReportTable extends AbstractTableDialog {
    /** ロガー */
    private static final LoggerHolder LOG = new LoggerHolder(DropOrCombatReportTable.class);
     ...
     * @param parent
     */
    public DropOrCombatReportTable(Shell parent, MenuItem menuItem) {
    ...
    protected void createContents() {
        ...
        reloadDB.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                MessageBox box = new MessageBox(DropOrCombatReportTable.this.shell, SWT.YES | SWT.NO
                        | SWT.ICON_QUESTION);
                ...
                if (box.open() == SWT.YES) {
                    ...
                    DropOrCombatReportTable.this.reloadTable();
                }
            }
        });
        ...
        this.table.addMouseListener(new MouseAdapter() {
            ...
            public void mouseDoubleClick(MouseEvent e) {
                int selected = DropOrCombatReportTable.this.table.getSelectionIndex();
                if (selected != -1) {
                    BattleResultDto result = DropOrCombatReportTable.this.getItemFromIndex(selected);
                    DropOrCombatReportTable.this.detailDialog.open();
                    DropOrCombatReportTable.this.detailDialog.setBattle(
                            DropOrCombatReportTable.this.getHTMLOfItem(result, false),
                            DropOrCombatReportTable.this.getBattleTitle(result));
                }
            }
        });
        SelectionListener filterListener = new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (DropOrCombatReportTable.this.battleFilterDialog == null)
                    DropOrCombatReportTable.this.battleFilterDialog =
                            new BattleFilterDialog(DropOrCombatReportTable.this);
                DropOrCombatReportTable.this.battleFilterDialog.open();
            }
        };
        if (!this.isNoMenubar()) {
            // フィルターメニュー
    ...
    private BattleResultDto getItemFromIndex(int index) {
        TableRowHeader rowHeader = (TableRowHeader) DropOrCombatReportTable.this.body.get(index)[0];
    ...
    private class SaveAdapter extends SelectionAdapter {
        ...
        @Override
        public void widgetSelected(SelectionEvent e) {
            if (DropOrCombatReportTable.this.table.getSelectionIndex() == -1) {
                // 選択されていないときは返る
                return;
            }

            List<BattleResultDto> selectedItems = DropOrCombatReportTable.this.getSelectedItemList();
            Shell shell = DropOrCombatReportTable.this.getShell();
            FileDialog dialog = new FileDialog(shell, SWT.SAVE);
            String fileName = DropOrCombatReportTable.this.getOutputFileName(selectedItems.get(0));
            ...
            if (filename != null) {
                ...
                if (selectedItems.size() > 1) {
                    DropOrCombatReportTable.this.writeToZipFile(selectedItems, file);
                }
                else {
                    DropOrCombatReportTable.this.writeToFile(selectedItems.get(0), file);
                }
            }
        }
